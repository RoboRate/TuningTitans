    ...
    with open(dataset_path, "r", encoding="utf-8") as jsonl_file:
        for line in jsonl_file:
            ...
            for model_name in model_names:
                ...
                model_answers[model_name].append(predicted_completion)
            questions.append(prompt)
            ideal_answers.append(expected_completion)

    cust_prod_info = {
        'Question': questions,
        'ModelAnswers': model_answers,
        'IdealAnswer': ideal_answers
    }
...
def evalWithRubric(test_set, model_names):
    ...
    for model_name in model_names:
        rubric_evals = []
        for question, your_model_answer, ideal_answer in zip(questions, model_answers[model_name], ideal_answers):
            user_message = f"""\
                你正在針對問題來做{model_name}的實際回答以及標準回答間的差異評分，並且依照評分邏輯做為判斷邏輯。
                以下是數據：
                [評分資料開始]
                ************
                [問題]: {question}
                ************
                [學生的實際回答]: {your_model_answer}
                ************
                [標準回答]: {ideal_answer}
                ************
                [評分邏輯]：
            ...
            response = getCompletionFromMessages(messages, model=model_name)
            rubric_evals.append(response)
        rubric_evaluations[model_name] = rubric_evals
    return rubric_evaluations

############################本地端測試
# if __name__ == "__main__":
#     dataset_path = "C:\\Users\\Sherry Wu\\.vscode\\Roborate_OpenAI\\QA.jsonl"
#     model_name = "ada:ft-personal-2023-08-14-06-31-09"
